...
SPAM_COOLDOWN    = 1800  # 30 دقيقة بين كل إشارة
...
_cache = {}
CACHE_TTL = 900  # ✅ رُفع من 600 إلى 900 ثانية (15 دقيقة) لتقليل الطلبات
...
def get_data(asset="BTC", days=30, interval="hourly"):
    ...
    if TWELVEDATA_KEY:
        try:
            ...
            if "values" in data and len(data["values"]) > 0:
                ...
                logger.warning("Twelve Data returned no values for " + symbol + ": " + str(data.get("message", "")))
        except Exception as e:
            logger.warning("Twelve Data failed: " + str(e))

    # ✅ Fallback: CoinGecko للبيتكوين فقط (مع retry delay)
    if asset == "BTC":
        try:
            import time
            time.sleep(2)  # ✅ تأخير بسيط لتفادي rate limit
            r = requests.get(
            ...
            if "prices" not in data:
                logger.error("CoinGecko bad response: " + str(data))
                return None
            ...
            result = df.dropna()
            # ✅ للبيانات الساعية: resample من البيانات اليومية
            if interval == "hourly":
    ...
    # ✅ Fallback للذهب: Twelve Data بـ outputsize أصغر
    if asset != "BTC" and TWELVEDATA_KEY:
        try:
            ...
            logger.warning("Twelve Data XAU fallback failed: " + str(e))

    logger.error(asset + " — all data sources failed")
    return None
...
def calc_indicators(df):
    ...
    h = df['High'].copy()
    l = df['Low'].copy()
    df = df.copy()  # ✅ منع SettingWithCopyWarning
    ...
    try:
        ...
        df['Vol_High'] = False
    try:
        o = df['Open']; hh = df['High']; ll = df['Low']; cc = df['Close']
        body   = abs(cc - o)
        candle = hh - ll
        df['PinBar_Bull'] = ((ll - cc.combine(o, min)) > body * 2) & (candle > 0)
        df['PinBar_Bear'] = ((hh - cc.combine(o, max)) > body * 2) & (candle > 0)
        df['Engulf_Bull'] = (cc > o.shift(1)) & (o < cc.shift(1)) & (cc.shift(1) < o.shift(1))
        df['Engulf_Bear'] = (cc < o.shift(1)) & (o > cc.shift(1)) & (cc.shift(1) > o.shift(1))
        df['HH'] = hh > hh.shift(1)
        df['LL'] = ll < ll.shift(1)
    except:
...
def analyze_frame(df, uid=0):
    ...
    macd_s   = last['MACD_S'] if not pd.isna(last['MACD_S']) else 0
    macd_h   = last['MACD_H'] if not pd.isna(last['MACD_H']) else 0
    if macd_val > macd_s and macd_h > 0:
        sb += 20; details.append(t(uid,'ind_macd_pos'))
    elif macd_val < macd_s and macd_h < 0:
        ss += 20; details.append(t(uid,'ind_macd_neg'))

    ema9  = last['EMA9']  if not pd.isna(last.get('EMA9',  float('nan'))) else price
    ema21 = last['EMA21'] if not pd.isna(last.get('EMA21', float('nan'))) else price
    ema50 = last['EMA50'] if not pd.isna(last.get('EMA50', float('nan'))) else price
    if ema9 > ema21 > ema50:
    ...
    if not pd.isna(ema200):
        if price > ema200: sb += 10
        else: ss += 10

    bb_l = last['BB_L'] if not pd.isna(last.get('BB_L', float('nan'))) else price * 0.98
    bb_u = last['BB_U'] if not pd.isna(last.get('BB_U', float('nan'))) else price * 1.02
    if price <= bb_l:   sb += 15; details.append(t(uid,'ind_bb_low'))
    elif price >= bb_u: ss += 15; details.append(t(uid,'ind_bb_high'))

    stoch   = last['Stoch']   if not pd.isna(last.get('Stoch',   float('nan'))) else 50
    stoch_s = last['Stoch_S'] if not pd.isna(last.get('Stoch_S', float('nan'))) else 50
    if stoch < 20 and stoch_s < 20:
        sb += 10; details.append(t(uid,'ind_stoch_low'))
    elif stoch > 80 and stoch_s > 80:
    ...
    try:
        ...
        if not any(pd.isna(x) for x in [tenkan, kijun, span_a, span_b]):
    ...
    s1 = last['S1'] if not pd.isna(last.get('S1', float('nan'))) else price * 0.99
    r1 = last['R1'] if not pd.isna(last.get('R1', float('nan'))) else price * 1.01
    atr = last['ATR'] if not pd.isna(last.get('ATR', float('nan'))) else price * 0.01

    return {
        "direction": direction, "conf": conf, "sb": sb, "ss": ss,
        "rsi": round(rsi, 1), "price": round(float(price), 2),
        "atr": round(float(atr), 2), "details": details[:4],
        "support": round(float(s1), 2), "resistance": round(float(r1), 2),
...
def detect_market_regime(df):
    try:
        ...
        atr_pct = float(last["ATR"]) / price * 100 if not pd.isna(last.get("ATR", float('nan'))) else 1.0
        ...
        bb_u = float(last.get("BB_U", price * 1.02))
        bb_l = float(last.get("BB_L", price * 0.98))
...
def detect_rsi_divergence(df, lookback=20):
    try:
        ...
        if any(pd.isna(rsi)):
            return "NONE"
        price_hh = prices[-1] > max(prices[:-5])
        price_ll = prices[-1] < min(prices[:-5])
        rsi_lh   = rsi[-1]   < max(rsi[:-5])
        rsi_hl   = rsi[-1]   > min(rsi[:-5])
    ...
    try:
        ...
        for i in range(2, len(recent) - 2):
            ...
            if candle["Close"] < candle["Open"]:
                if all(next3["Close"] > next3["Open"]) and \
                   next3["Close"].max() > candle["Open"] * 1.005:
                    bullish_obs.append({
            ...
            if candle["Close"] > candle["Open"]:
                if all(next3["Close"] < next3["Open"]) and \
                   next3["Close"].min() < candle["Open"] * 0.995:
                    bearish_obs.append({
...
async def monitor_btc(context):
    ...
    try:
        ...
        to_remove = []
        for trade in list(active_trades):  # ✅ iterate على نسخة لتفادي mutation
            try:
                current = current_btc
...
async def send_smart_alerts(context):
    try:
        for asset in ["BTC"]:
            ...
            last  = df.iloc[-1]
            price = float(last['Close'])
            rsi   = float(last['RSI']) if not pd.isna(last.get('RSI', float('nan'))) else 50
            atr   = float(last['ATR']) if not pd.isna(last.get('ATR', float('nan'))) else price * 0.01
