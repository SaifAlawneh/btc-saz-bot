...
AUTO_INTERVAL_MIN = 30   # كل نص ساعة
MONITOR_MIN       = 5    # مراقبة الصفقات المفتوحة كل 5 دقائق
MIN_CONFIDENCE    = 68
...
# { asset: { "entry":x, "sl":x, "tp1":x, "tp2":x, "tp3":x, "direction":x, "msg_chat":x, "msg_id":x } }
active_trades = {}
...
REPLIES_AR = [
    "هلا وغلا! 🐎 أنا بوت أبو مهرة جاهز أحللّك السوق\nاختر من القائمة 👇",
    "أهلاً بك! 🤖 تبي صفقة ولا تحليل؟ اختر 👇",
    "وعليكم السلام! 🐎 شو أقدر أساعدك؟",
    "هلا! 😊 اضغط على أي زر للبدء 👇",
    "يسعد مساك/صباحك! 🌟 تبي صفقة BTC أو الذهب؟",
]
REPLIES_EN = [
    "Hello! 🐎 I'm Abu Mahra Bot, ready to analyze!\nUse the buttons below 👇",
    "Hi there! 🤖 Want a trade or analysis? Choose 👇",
    "Hey! 😊 Press any button to get started 👇",
    "Welcome! 🌟 BTC or Gold trade signal?",
]
CONFUSED_AR = [
    "ما فهمت كثير 😅 استخدم الأزرار للمساعدة 👇",
    "🤔 أنا بوت تداول! اختر من القائمة 👇",
    "أنا بوت أبو مهرة 🐎 متخصص في BTC والذهب 👇",
]
CONFUSED_EN = [
    "Didn't quite get that 😅 Use the buttons below 👇",
    "🤔 I'm a trading bot! Use the menu 👇",
    "I'm Abu Mahra Bot 🐎 BTC & Gold signals! 👇",
]
...
⚠️ للأغراض التعليمية فقط""",
        ...
        "update_header": "🔄 تحديث الصفقة - أبو مهرة",
        ...
        "timeframe": "⏱️ الفريم", "hold_time": "⏳ المدة",
        ...
        "update_tp1_hit":    "✅ الهدف الأول تم! نقل SL للدخول",
        "update_tp2_hit":    "✅✅ الهدف الثاني تم! نقل SL للـ TP1",
        "update_near_sl":    "⚠️ تحذير: السعر اقترب من وقف الخسارة",
        "update_reversal":   "🔄 إشارة عكس محتملة — راجع الصفقة",
        "current_price":     "💵 السعر الحالي",
        "new_sl":            "🛑 وقف خسارة جديد",
        "action_suggested":  "💡 الإجراء المقترح",
...
🔄 مراقبة وتحديث SL/TP كل 5 دقائق
⚙️ توافق 3 فريمات — إشارة فقط عند توافق فريمين+
⚠️ للأغراض التعليمية فقط""",
...
▫️ Live SL/TP monitoring & updates
...
⚠️ For educational purposes only""",
        ...
        "update_header":   "🔄 Trade Update - Abu Mahra",
        ...
        "timeframe": "⏱️ Timeframe", "hold_time": "⏳ Hold Time",
        ...
        "update_tp1_hit":    "✅ TP1 reached! Moved SL to entry",
        "update_tp2_hit":    "✅✅ TP2 reached! Moved SL to TP1",
        "update_near_sl":    "⚠️ Warning: Price approaching Stop Loss",
        "update_reversal":   "🔄 Possible reversal signal — review trade",
...
🔄 Live SL/TP monitoring every 5 minutes
...
def get_data(asset="BTC", days=7, interval="hourly"):
    try:
        ...
        r    = requests.get(url, params={"vs_currency":"usd","days":days,"interval":interval}, timeout=15)
        data = r.json()
...
def scalp_analysis(asset="BTC", uid=0):
    """تحليل للصفقات القصيرة - 3 فريمات"""
    frames = {
    ...
    # أهداف Scalp - ضيقة وسريعة بـ ATR ساعي
    if final=="BUY":
...
def build_update_msg(trade, current_price, update_type, uid=0):
    """رسالة تحديث الصفقة"""
    ai = "₿" if trade['asset']=="BTC" else "🥇"
    an = "BTC/USD" if trade['asset']=="BTC" else "XAU/USD"
...
━━━━━━━━━━━━━━━━━━━━━━━━
{t(uid,'direction')}:  {dir_txt}
{t(uid,'entry')}:  ${trade['entry']:,.2f}
{t(uid,'current_price')}:  ${current_price:,.2f}
...
async def handle_message(update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(get_reply(uid, text), reply_markup=kb)
...
async def button_handler(update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data=='lang_ar':
        ...
        try:
            ...
            msg_txt=build_trade_msg(res,uid)
            sent=await query.message.reply_text(msg_txt)
            # حفظ الصفقة للمراقبة
            active_trades[asset]={
                "asset":asset,"direction":res['final'],
                "entry":res['price'],"sl":res['sl'],
                "tp1":res['tp1'],"tp2":res['tp2'],"tp3":res['tp3'],
                "atr":res['atr'],"tp1_hit":False,"tp2_hit":False,
                "chat_id":query.message.chat_id,
            }
            logger.info(f"📌 صفقة {asset} مفتوحة - {res['final']} @ {res['price']}")
        except Exception as e:
...
# ==================== مراقبة الصفقات كل 5 دقائق ====================
async def monitor_trades(context):
    """يراقب الصفقات المفتوحة ويبعث تحديثات عند تغير مهم"""
    ...
    for asset, trade in list(active_trades.items()):
        try:
            ...
            if direction == "BUY":
                ...
                # trailing stop — لو تحرك كثير نحرك SL
                ...
                # TP3 تم — نغلق الصفقة
                if current >= tp3:
                    update_msg = f"🏆 الهدف الثالث تم! صفقة {asset} مغلقة بنجاح"
            ...
            if update_msg:
                msg = build_update_msg(trade, current, update_msg, uid)
                await context.bot.send_message(chat_id=chat_id, text=msg)
                logger.info(f"🔄 {asset} Update: {update_msg}")

        except Exception as e:
            logger.error(f"❌ Monitor {asset}: {e}")
...
def main():
    ...
    # مراقبة الصفقات كل 5 دقائق
    app.job_queue.run_repeating(monitor_trades, interval=MONITOR_MIN*60, first=60)

    logger.info("🐎 Abu Mahra Bot - Live Monitor Edition!")
    app.run_polling()
